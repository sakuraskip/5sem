...
)
{
    ...
    if (host == NULL) {
        cout << "GetServerByName: host == null" << endl;
        return false;
    }
    ...
    server_addr.sin_port = ((sockaddr_in*)from)->sin_port; 
    memcpy(&server_addr.sin_addr, host->h_addr, host->h_length);

    cout << "resolved " << name << " ip: " << inet_ntoa(server_addr.sin_addr) << endl;

    if ((sendtoLength = sendto(*sock, call, strlen(call), NULL,
        (sockaddr*)&server_addr, sizeof(server_addr))) == SOCKET_ERROR) {
        throw SetErrorMsgText("GetServerByName sendto: ", WSAGetLastError());
    }

    cout << "Sent to server " << name << " message: " << call << endl;

   
    if ((inputLength = recvfrom(*sock, inputBuffer, sizeof(inputBuffer) - 1, 0, from, flen)) == SOCKET_ERROR) {
        if (WSAGetLastError() == WSAETIMEDOUT) {
            cout << "GetServerByName timed out" << endl;
            return false;
        }
        else throw SetErrorMsgText("GetServerByName recvfrom: ", WSAGetLastError());
    }

    inputBuffer[inputLength] = '\0';
    cout << "GetServerByName  recvfrom " << name << ": " << inputBuffer << endl;

    if (strcmp(inputBuffer, call) == 0) {
        sockaddr_in* from_in = (sockaddr_in*)from;
        from_in->sin_addr = server_addr.sin_addr;
        from_in->sin_family = AF_INET;
        from_in->sin_port = server_addr.sin_port;
