    ...
    HTHANDLE** result)
{
    ...
    hr = pIHT->Create(Capacity, SecSnapshotInterval, MaxKeyLength, MaxPayloadLength, FileName,HTUsersGroup, result);
    pIHT->Release();
    return hr;
}
    ...
    HTHANDLE** result)
{
 ...
 OS13_LIB_API HRESULT HTOpen(
     const wchar_t* htusername,
     const wchar_t* htuserpassword,
     const char* FileName,
     HTHANDLE** result)
 {
     IHT* pIHT = nullptr;
     ...
     hr = pIHT->Open(htusername,htuserpassword,FileName, result);
     pIHT->Release();
     return hr;
 }
