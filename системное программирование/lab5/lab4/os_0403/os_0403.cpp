...
{   
    ...
    while (active)
    {
        ...
        string newpayload = "funny" + to_string(i);
        ...
        if (SUCCEEDED(pIHT->Update(ht, e,newpayload.c_str(), newpayload.length())))
        {
            cout << "updated " << i << " element with key: " << key << endl;
        }
        else
        {
            cerr << "failed to update element " << i << "with key: " << key <<  endl;
        }
    ...
    hr = pIHT->Close(ht);
    if (FAILED(hr)) {
        cerr << "failed to close HT storage. HRESULT: 0x" << hex << hr << endl;
    }
    ...
    hr = UninitializeCOM();
    if (FAILED(hr)) {
        cerr << "failed to uninitialize COM. HRESULT: 0x" << hex << hr << endl;
    }
