...
int main(int argc, char** argv)
{
    ...
    while (active)
    {
        ...
        HT::Element* forDeletion = HT::Get(ht, e);

        if (forDeletion !=nullptr)
        {
            if (HT::Delete(ht, forDeletion))
            {
                cout << "\ndeleted item with key " << key << endl;
                delete forDeletion;
            }
            else
            {
                cout << "\ndeletion for item with key " << key << " failed" << endl;
            }
            
        }
        else
        {
            cout << "\nitem with key " << key << " not found for deletion" << endl;
        }
