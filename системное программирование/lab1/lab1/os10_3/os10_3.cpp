...
int main()
{
    ...
    try
    {
        ...
        //---------------------------------------------------------------------------------------
        ht1 = HT::Create(1000, 3, 10, 256, "test3.ht", 0);  
        if (ht1) std::cout << "-- Create: success";
        else throw " -- Create: error";

        if (HT::Insert(ht1, new HT::Element("key2223", 7, "data", 8))) std::cout << "-- Insert:success";
        else throw "-- insert:error";

        HT::Element* hte_1 = HT::Get(ht1, new HT::Element("key2223", 7));
        if (hte) std::cout << "Get: success";
        else throw "-- Get: error";

        /* HT::print(hte);*/

        if (HT::Snap(ht1)) std::cout << "Snap: success";
        else throw "-- Snap: error";

        if (HT::Update(ht1, hte_1, "newpayload", 11)) std::cout << "-- Update:success";
        else throw "-- Update:error";

        HT::Element* hte1_1 = HT::Get(ht1, new HT::Element("key2223", 7));
        if (hte1) std::cout << "Get: success";
        else throw "-- Get: error";

        //HT::print(hte1);

        if (HT::Delete(ht1, hte1_1)) std::cout << "Delete: success";
        else throw "-- Delete: error";
