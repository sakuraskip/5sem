...
afx_msg int CMainWin::OnCreate(LPCREATESTRUCT)
{
	...
	CMenu sub;
	sub.CreatePopupMenu();
	sub.AppendMenu(MF_STRING, ID_Dr, _T("Только видимые"));
	sub.AppendMenu(MF_STRING, ID_Dr1, _T("Все грани"));
...
afx_msg void CMainWin::OnSize(UINT nType, int cx, int cy)
{
	...
	if (cx > 0 && cy > 0) {
		int rectSize = min(cx, cy) * 0.8; 
		int left = (cx - rectSize) / 2;
		int top = (cy - rectSize) / 2;

		WinRect.SetRect(left, top, left + rectSize, top + rectSize);
	}
}
