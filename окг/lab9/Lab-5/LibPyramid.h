...
public:
	...
	void DrawLightSphere(CDC& dc, CMatrix& PView, CMatrix& PSourceLight, CRect RW, COLORREF Color);
};
...
//////////Рисует пирамиду с учётом диффузного освещения (модель Ламберта)
void CPyramid::ColorDraw(CDC& dc, CMatrix& PView, CRect& RW, COLORREF color)
{
	...
	CMatrix ViewVert = MV * Vertices; //Пересчитывает вершины пирамиды в видовые координаты
	CRectD RectView;
	...
	for (int i = 0; i < 3; i++) //////рисование боковых граней
	{
		...
		VN = VectorMult(V2, V1);		//Для каждой боковой грани вычисляет нормаль через векторное произведение рёбер
		sm = cosv1v2(VN, ViewCart);	    // cos (нормаль к грани, вектор к точке наблюдения) - косинус фи

		if (sm >= 0) // Если косинус угла между нормалью и вектором к наблюдателю (cosv1v2) ≥ 0, грань видима
		{
			CPen Pen(PS_SOLID, 2, RGB(sm * Red * 0.3, sm * Green * 0.3, sm * Blue * 0.3));	//контур
			CPen* pOldPen = dc.SelectObject(&Pen);
			//Цвет грани зависит от угла между нормалью и направлением света (интенсивность = sm * color)
...
void CPyramid::DrawLightSphere(CDC& dc, CMatrix& PView, CMatrix& PSourceLight, CRect RW, COLORREF Color)
{ //Использует координаты источника света (PSourceLight)
	BYTE red = GetRValue(Color);
	BYTE green = GetGValue(Color);
	BYTE blue = GetBValue(Color);
	double kLight;
	...
	CMatrix VS = SphereToCart(PSourceLight);//ист.света в декарт
	CMatrix MV = CreateViewCoord(PView(0), PView(1), PView(2));//М-ца -> ВСК
	CMatrix ViewVert = MV * Vertices; //вершины в видовой СК
	GetRect(ViewVert, RectView); //прям-к, охват проекцию пирамиды на XY ВСК
	CMatrix MW = SpaceToWindow(RectView, RW); // м-ца МСК->ОСК
	...
	CMatrix R1(3),	///тек. точка основания
		...
		VN(3);		///вектор нормали
	double sm;
	for (int i = 0; i < 3; i++) //боковые грани
	{
		...
		VN = VectorMult(V2, V1);
		sm = cosv1v2(VR, VN);	   // cos (N к грани, вект ТН) i = I*k*cos(Ik), видно ли наблюдателю?
		double UgolAlpha = Ugol(VS, VN);

		if (sm >= 0) // Грань видима – рисуем боковую грань
		{
			CMatrix VP = VS - VR; // Вектор от наблюдателя к источнику света
			sm = cosv1v2(VP, VN); //влияет ли свет на грань?
			double result = 0;
			if (sm > 0)
			{
	...
	if (sm >= 0) // Основание
	{
		...
		if (sm > 0)
		{
			...
			result = pow(kLight, 2); //интенсивность света для боковых граней
		}
	...
	else
	{
		...
		{
			kLight = cosv1v2(VN, VR);
			result = pow(kLight, 5); //интенсивность света для основания
		}
