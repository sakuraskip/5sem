...
//-------------------------------------------------------------------------------
struct CMyPen   //для создания пера
{
...
class CPlot2D   //зависимость Yi=f(Xi)
{
	CMatrix X;		// Аргумент
	CMatrix Y;		// Функция
...
public:
	CPlot2D() {
		K.RedimMatrix(3, 3);		// Изменяет размер матрицы с сохранением данных, которые можно сохранить
	};
	...
	void Draw1(CDC& dc, int Ind1, int Int2);	// Рисование с БЕЗ самостоятельного пересчетa координат
